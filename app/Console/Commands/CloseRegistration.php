...
class CloseRegistration extends Command
{
     ...
     * @var string
     */
    protected $description = 'Command used to close registration';
    ...
    public function handle()
    {
        $data = $this->rankSaw();
        for($i = 0; $i < 100; $i++) {
            Mahasiswa::find($data[$i]["mahasiswa"]->id)
                ->update([
                    "status" => "Lulus"
                ]);
        }
        for($i = 100; $i < count($data); $i++) {
            Mahasiswa::find($data[$i]["mahasiswa"]->id)
                ->update([
                    "status" => "Tidak Lulus"
                ]);
        }
        echo "Registration is closed, 100 students have been noted to be passed";
    }
